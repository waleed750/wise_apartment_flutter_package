...
- (void)emitEvent:(NSDictionary *)event {
    if (!event || ![event isKindOfClass:[NSDictionary class]]) {
        NSLog(@"[WAEventEmitter] Invalid event format: %@", event);
        return;
    }
    
    // Validate event has "type" key
    if (!event[@"type"]) {
        NSLog(@"[WAEventEmitter] Event missing 'type' key: %@", event);
        return;
    }
    ...
    dispatch_async(self.eventQueue, ^{
        if (self.eventSink) {
            ...
            NSLog(@"[WAEventEmitter] No active event sink, dropping event: %@", event[@"type"]);
        }
    });
}
