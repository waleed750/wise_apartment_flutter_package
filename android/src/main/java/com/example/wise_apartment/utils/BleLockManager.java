...
public class BleLockManager {
    ...
    /**
     * Callback interface for streaming WiFi registration events.
     */
    public interface WifiRegistrationStreamCallback {
        void onChunk(Map<String, Object> event);
        void onDone(Map<String, Object> event);
        void onError(Map<String, Object> event);
    }
...
//        syncLockRecordAction.setBaseAuthAction(baseAuth);
    ...
    /**
     * Streaming variant of registerWifi. Invokes the SDK and reports
     * completion/errors via the provided callback. The plugin layer should
     * acknowledge the method call immediately and forward events to Flutter
     * through the EventChannel.
     */
    public void registerWifiStream(Map<String, Object> args, final WifiRegistrationStreamCallback callback) {
        ...
        // Build baseAuth same as registerWifi
        com.example.hxjblinklibrary.blinkble.entity.requestaction.BlinkyAuthAction baseAuth = null;
        try {
            String macFromArgs = null;
            if (args != null) {
            ...
            Log.e(TAG, "Failed to build baseAuth from dna map or mac (stream), falling back", t);
            baseAuth = PluginUtils.createAuthAction(args);
        }
        ...
        try {
            bleClient.rfModuleReg(action, wifiJson, new FunCallback() {
                ...
                public void onResponse(Response rfResp) {
                    try {
                        ...
                        if (callback != null) {
                            ...
                            done.put("type", "wifiRegistrationDone");
                            done.put("response", out);
                ...
                @Override
                public void onFailure(Throwable t) {
                    Log.e(TAG, "registerWifiStream rfModuleReg failed", t);
                    if (callback != null) {
                        Map<String, Object> err = new HashMap<>();
                        err.put("type", "wifiRegistrationError");
                        err.put("message", t.getMessage());
            ...
            Log.e(TAG, "Exception calling rfModuleReg in registerWifiStream", t);
            if (callback != null) {
                Map<String, Object> err = new HashMap<>();
                err.put("type", "wifiRegistrationError");
                err.put("message", t.getMessage());
